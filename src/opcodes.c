...
void opcode_1nnn_handler(uint16_t n) { program_counter = n; }
...
void opcode_annn_handler(uint16_t n) { index_register = n; }
...
void opcode_fx07_handler(uint8_t reg) { registers[reg] = delay_register; }
...
void opcode_fx15_handler(uint8_t reg) { delay_register = registers[reg]; }

// LD ST, VX — FX18
// Sets the sound timer from VX.
void opcode_fx18_handler(uint8_t reg) { sound_register = registers[reg]; }
